...
local function new(self)
  ...
  -- @rationale returns the value of the Nginx variable `$scheme`.
  ...
  -- @phases `rewrite`, `access`, `header_filter`, `body_filter`, `log`
  -- @rationale returns the value of the Nginx variable `$host`.
  ...
  -- @rationale returns the value of the Nginx variable `$server_port`.
  ...
  -- @phases `rewrite`, `access`, `header_filter`, `body_filter`, `log`
  -- @rationale
  -- Avoids the "uri" misnomer for referring to paths that is
  -- present everywhere in NGINX (and, by extension, OpenResty).
  ...
  -- @phases `rewrite`, `access`, `header_filter`, `body_filter`, `log`
  -- @rationale
  -- Provides the raw querystring for users who wish to
  -- perform matching in a different way than we do in `get_query`. This
  -- gives us more flexibility in defining the semantics of `get_query`.
  ...
  -- @phases `rewrite`, `access`, `header_filter`, `body_filter`, `log`
  -- @rationale
  -- Our version of `ngx.req.get_uri_args`, with a better name. The
  -- OpenResty version also allows `max_args` to be 0 (unlimited) although that is
  ...
  -- @phases `rewrite`, `access`, `header_filter`, `body_filter`, `log`
  -- @rationale
  -- essentially `ngx.req.get_headers`. The OpenResty version also
  -- allows `max_headers` to be 0 (unlimited) although that is "strongly
  -- discouraged".
  ...
  -- @rationale
  -- Based on `ngx.req.get_body_data`. Note that Kong exposes
  -- the Nginx's `client_body_buffer_size` setting in its configuration, which is
  -- the threshold for reading via memory or a file. Our plugins use
  -- `ngx.req.get_body_data` only, and don't work with large files unless this
  -- directive is increased from its default – making them work with files could
  -- make them unacceptably slow in some case.  Unlike the OpenResty version, in
  -- case of an empty body, we return an empty string and not `nil`.
