...
-- *As a plugin author*:
--
-- * I should not be required to browse the `ngx_lua` or OpenResty API
--   documentation. All that I need should be documented on https://getkong.org
--   (or future documentation website).
-- * I should not need to write "OpenResty-like" Lua (using the `ngx.*` global). I
--   should enjoy a more native experience with a high-level `kong.*` global.
-- * I should still be able to access the low-level `ngx_lua` API (using the
--   `ngx.*` global) for more technical/precise needs than the ones offered by the
--   PDK.
-- * I should be confident in expecting a degree of backwards-compatibility from
--   the provided said PDK between Kong versions. The semver semantics used to
--   version the PDK should allow my plugin, written with a previous version of
--   said PDK, to still function granted it can access its desired major version.
--
-- *As a Kong employee (mostly engineering or support)*:
--
-- * We wish to eventually defer all "how do I do X" questions to the PDK's
--   documentation (How do I make an HTTP request? How do I write to a file?).
-- * No other languages than Lua are to be supported for now.
-- * For now, only "stateless" functions are to be specified. No changes to the
--   way plugins are written (the interface to implement or the mandatory files)
--   should be made.
-- * None of our existing plugins should break as a result of this new PDK. It
--   should only be a super-set of the OpenResty APIs, and the latter should still
--   be available (as previously stated).
-- * When possible, respect the OpenResty philosophy of "no added overhead" for
--   those modules - reasonably preserve performance, but make usability a higher
--   priority.
--
-- @module kong
-- @release 0.1.1 (RFC)
